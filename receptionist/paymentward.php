...
if(isset($_POST['submit_cash1'])){
	...
	if($stmt->execute()){
		...
		$sql_updatePayment = "UPDATE medical_history SET payment_status = 'Complete', end_warded = '".$date."' WHERE history_id = ".$history_id."";
...
if(isset($_POST['submit_insurance1'])){
	$method = 'insurance';
	$history_id = $_POST['history_id'];
	$total_price2 = $_POST['totalPrice'];
...
if (isset($_GET['paymentward']) && isset($_GET['id'])) {
	$history_id = $_GET['id'];
				...
				<div style="display: none" id="cash">
					...
					<div>
						...
						<form method="POST">
								...
								<input name="totalPrice" type="hidden" type="text" value="<?php echo $total_price2; ?>">
								<button name="submit_cash1" class="btn btn-success" style="width: 100%">Payment Completed</button>
							</form>
						</div>
					</div>
				</div>
					...
					<table class="table">
						...
						<div>
							<form method="POST">
								<input name="history_id" type="hidden" type="text" value="<?php echo $history_id; ?>">
								<input name="totalPrice" type="hidden" type="text" value="<?php echo $total_price2; ?>">
								<button <?php if($row_insurance2['insurance_status'] === 'Not Active'){echo 'disabled';} ?> name="submit_insurance1" class="btn btn-success" style="width: 100%">Issued to Insurance</button>
							</form>
						</div>
					</div>
				</div>
